...
class InvoicePrinter extends FPDF
{
    ...
    public function leftTitle($text){
        $this->leftTitle = $text;
    }

    public function rightTitle($text){
    ...
    public function Header()
    {
        ...
        if ($this->PageNo() == 1) {
            ...
            if ($this->display_tofrom === true) {
                $this->Cell($width, $lineheight, iconv("UTF-8", "ISO-8859-1", mb_strtoupper($this->leftTitle, 'UTF-8')), 0, 0, 'L');
                $this->Cell(0, $lineheight, iconv("UTF-8", "ISO-8859-1", mb_strtoupper($this->rightTitle, 'UTF-8')), 0, 0, 'L');
                $this->Ln(7);
