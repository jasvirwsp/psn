...
if($test["status"] == "failure"){
	...
	$status_array = array("message"=>$has_errors["errors"][0],"code"=>"401");
	$response["response"] = $status_array;    
	$final_json = json_encode($response);
	print_r($final_json);
		exit();
...
//Hook
//before_insert_trash($_POST,$record_type);
//Hook

//Get meta keys of record type and prepare insert_array for insertion
$meta_keys_array_name = "meta_keys_of_".$record_type;
$meta_keys_array = $$meta_keys_array_name;
$insert_record_array = array("date_created"=>$_POST["date_created"],"date_updated"=>$_POST["date_updated"]);
...
$record_id = $insert_id;
	...
	//Hook
	//after_insert_trash($_POST,$response,$record_type);
	//Hook
