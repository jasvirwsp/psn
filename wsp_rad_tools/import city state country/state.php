                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title"><?php echo beautify_meta_name(title_case("state")); ?> <!-- Button trigger modal -->
<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#add_state"><i class="fe-plus"></i> Add <?php echo beautify_meta_name(title_case("state"));?></button>
</h4>
                                </div>
                            </div>
                        </div>     
...
//Action Buttons
$edit_button_url = "edit_state.php?state_id=";
$view_button_url = "view_state.php?state_id=";
$delete_button_class = "delete_state";
//Records
$per_page = 20;
//Offset settings
...
$where_array = array("state_name != ?"=>""); // You can change it as per required
...
<table class="table table-striped" id="list_table_state">
<?php
...
<!-- Col 1 -->
        ...
        <a href="javascript:void" class="btn btn-secondary btn-sm export_to_csv_state"><i class="fa fa-download" aria-hidden="true"></i> CSV</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm export_to_xls_state"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm print_table_state"><i class="fa fa-print" aria-hidden="true"></i> Print</a>
        </div>
<!-- Col 1 ends -->
</div>
<!-- Row Ends -->
...
<!-- Add state Start-->
<div class="modal fade" id="add_state" tabindex="-1" role="dialog" aria-labelledby="add_state_LongTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Add <?php echo beautify_meta_name(title_case("state"));?></h5>
      ...
      <!-- name>i --&gt<!-- Form Row Starts -->
        <div class="row">
        <form method="post" action="" id="form_state_rad5df9bba2f1785" class="w-100">
 <div class="form-group ic_state_name col-md-5 d-inline-block">
         <label class="label_state_name" for="state_name">Name</label>
        <input type="text" class="form-control state_name" placeholder="Name"  name="state_name">
            </div>
        
        <div class="form-group col-md-12">
        <input type="submit" class="btn btn-success btn-sm insert_state" value="Add State" form_id="rad5df9bba2f1785"> 
        </div>
        </form>
        </div>
        <!-- Form Row Ends -->
...
<!-- Quick Edit Modal for state Start-->
<div class="modal fade" id="quick_edit_modal_state" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Update <?php echo beautify_meta_name(title_case("state")); ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body ic_age_modal">
      <form method="post" action="" id="form_update_state_quick_edit" class="w-100">
      <input type="hidden" name="record_id" value="" class="quick_edit_state_record_id">
      <div class="modal_input_loader_state">

      </div>
      <div class="form-group col-md-12">
       
        <input type="submit" class="btn btn-success btn-sm update_state" value="Update <?php echo beautify_meta_name(title_case("state")); ?>" form_id="quick_edit"> 
        </div>
        </form>
      </div>
      <div class="modal-footer">
...
<!-- Quick Edit Modal state Ends-->
        ...
        //Insert variables
        var insert_button_class_state = ".insert_state"; // .classname
                var form_id_state = "#form_state";  // #entry_form
                var insert_api_url_state = "api/state/insert_state.php"; // /api/insert.php
                var form_id_parameter_name = "form_id";
                
                //Insert Operation
                $(document).on("click",insert_button_class_state,function(e){                
                e.preventDefault();
                //Processing
                swal("Processing","Please Wait","warning");
                //Processing
                var form_to_process_state = form_id_state + "_" + $(this).attr(form_id_parameter_name);
        
                $.post(insert_api_url_state,$(form_to_process_state).serializeArray(),function(data){
              ...
              icon: "error"
                    ...
                    $(form_to_process_state + " input[type=text]").first().focus();
...
customized_content_state.innerHTML = "<a href='edit_state.php?state_id="+entry_id_state+"' class='btn btn-sm btn-warning mt-1'>Edit State</a> <a href='#' onclick='location.reload()' class='btn btn-sm btn-warning mt-1'>Add New State</a> <!--<a href='#' class='btn btn-sm btn-success mt-1'>Another Action</a> <a href='#' class='btn btn-sm btn-info mt-1'>Another Action</a> --><br><br><a href='#' onclick='var loc = location.href.replace(location.hash,\"\");window.location = loc;' class='btn btn-sm btn-danger'>Close & Refresh</a> <a href='#' onclick='swal.close()' class='btn btn-sm btn-danger'>X</a>";
swal({
    title: "Success",
...
var records_table_id = "#list_table_state";
$(".export_to_csv_state").click(function(){
$(records_table_id).tableToCSV();
});

$(".export_to_xls_state").click(function(){
...
$(".print_table_state").click(function(){
...
var delete_record_button_class_state = ".delete_state"; // .classname
var primary_record_key_attribute_name_state = "record_id"; //Name of attribute of particular record to edit/update/view/delete.
var delete_api_url_state = "api/state/delete_state.php"; // /api/delete.php

$(document).on("click",delete_record_button_class_state,function(e){
e.preventDefault();
...
.then((willDelete) => {
if (willDelete) {
  var record_id_state = $(this).attr(primary_record_key_attribute_name_state);
    
  $.post(delete_api_url_state,{"record_id":record_id_state});
...
.then((willDelete) => {
  ...
  var api_url = "api/state/delete_state.php"; //Replace with correct API url
...
//Quick Edit Code for state starts
$(".quick_edit_state").click(function(){
          var quick_edit_state_record_type = 'state';
          var quick_edit_state_record_id = $(this).attr("record_id");
          $(".quick_edit_state_record_id").val(quick_edit_state_record_id);
          var quick_edit_state_input_name = $(this).attr("quick_edit_input_name");
          $("#quick_edit_modal_state").modal("show");
          $(".modal_input_loader_state").load("edit_"+quick_edit_state_record_type+".php?"+quick_edit_state_record_type+"_id="+quick_edit_state_record_id+" .ic_"+quick_edit_state_input_name);
        });
        

//Update Variables for state
var qe_update_button_class_state= ".update_state"; // .classname
var qe_form_id_state = "#form_update_state";
var qe_update_api_url_state = "api/state/update_state.php"; // /api/update.php
 var qe_form_id_parameter_name_state = "form_id";
    //Update Operation
    	   $(document).on("click",qe_update_button_class_state,function(e){
e.preventDefault();
    		  var qe_form_to_process_state = qe_form_id_state + "_" + $(this).attr(qe_form_id_parameter_name_state);

    	 $.post(qe_update_api_url_state,$(qe_form_to_process_state).serializeArray(),function(data){
        var qe_response_json_state = data;

if(qe_response_json_state["status"] == "failure"){
  var qe_get_errors_state = qe_response_json_state["errors"];
 
    var errors_returned_state = "";
  $(qe_get_errors_state).each(function(index,value){
            errors_returned_state = errors_returned_state + " => " + value;
    });
    ...
    swal("Error",errors_returned_state, {
      icon: "error"
        ...
        //Reset Form
    		//$(qe_form_to_process_state)[0].reset();

        

    		 // Other functions to execute on success
    		   //Customized buttons
var qe_customized_content_state = document.createElement("div");
qe_customized_content_state.innerHTML = "<a href='javascript:void(0)' onClick='location.reload()' class='btn btn-sm btn-warning mt-1'>Refresh Changes</a> <br><br><a href='javascript:void(0)' onclick='swal.close()' class='btn btn-sm btn-danger'>Close Box</a>";
swal({
    title: "Updated",
    text: "Choose next action",
  content: qe_customized_content_state,
  buttons: false,
  icon : 'success',
});
//Customized buttons
...
//Quick Edit Code for state ends 
        </script>
