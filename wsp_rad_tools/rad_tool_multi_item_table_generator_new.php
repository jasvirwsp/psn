...
<!-- Col 2 -->
        ...
        <?php
        if(isset($_POST["inputs_to_generate"])){ ?>
        <?php $mega_gen_input_form = '&lt;!-- Multi Item Table for '.beautify_meta_name($rec_type).'  --&gt;'; ?>
...
$mega_gen_input_form .='
&lt;td&gt;
    &lt;a href="javascript:void(0)" class="btn btn-danger multi_item_'.$rec_type.'_remove_item mb-1 mr-1" item_no="1"&gt;&lt;i class="fe-trash"&gt;&lt;/i&gt;&lt;/a&gt; &lt;/td&gt; 
';
?>
...
&lt;div class="multi_item_stats_'.$rec_type.' text-right bg-light p-2 mb-3"&gt;
                        ...
                        <!-- Box -->
                        &lt;div class="form-group ic_multi_item_total_items_'.$rec_type.' col-md-2 d-inline-block"&gt;
         &lt;label class="multi_item_total_items_label_'.$rec_type.'" for="multi_item_total_items"&gt;Total Items&lt;/label&gt;
        &lt;input type="number" class="form-control multi_item_total_items_'.$rec_type.'" placeholder="Total Items" readonly value="1"&gt;
            &lt;/div&gt;     
            <!-- Box -->

            <!-- Box -->
            &lt;div class="form-group ic_multi_item_total_quantity_'.$rec_type.' col-md-2 d-inline-block"&gt;
         &lt;label class="multi_item_total_quantity_label_'.$rec_type.'" for="multi_item_total_quantity"&gt;Total Qty&lt;/label&gt;
        &lt;input type="number" class="form-control multi_item_total_quantity_'.$rec_type.'" placeholder="Total Items" readonly value="0"&gt;
            &lt;/div&gt;     
            <!-- Box -->
            &lt;/div&gt;
                          <!-- Multi Item Stats Bar -->
...
$(document).on("click",".multi_item_<?php echo $rec_type;?>_remove_item",function(){
    ...
    if(items_length_<?php echo $rec_type;?> == 1){
...
<h3>Add this to regex_input_array in parent rec type insert file .</h3>

<textarea class="select_this form-control" cols="10" rows="2">
$regex_user_input = array("<?php echo $rec_type;?>"=>"");
</textarea>
<h3>Insert PHP Code for Hook</h3>
<textarea class="select_this form-control" cols="10" rows="20">
...
if($count_s == 1){
    $insert_array_string .= '"'.$html_name.'"=>$response["record_id"],';
}else{
