...
<textarea class="enter_field form-control" name="inputs_to_generate" rows="10" placeholder="Enter Input Names Separatedy by , comma" cols="20"><?php if(isset($_POST["inputs_to_generate"])) { echo $_POST["inputs_to_generate"]; }?></textarea>
<br>
...
<h3>Total Fields : <?php echo $inputs_count; ?></h3>
<!-- Row Starts -->
<div class="row">
<!-- Col 1 -->
        <div class="col-sm-6 d-none" style="width:49%;">
        <?php
...
$input_skeleton = '<li class="item-content item-input {input_name}_container">
<div class="item-media {input_name}_media">
    <i class="f7-icons">chevron_right_square</i>
</div>
<div class="item-inner {input_name}_content">
  <div class="item-title item-label {input_name}_label">{input_name_human}</div>
  <div class="item-input-wrap">
    <input type="{input_type}" placeholder="Your name" class="{input_name}" name="{input_name}">
...
?>
<textarea class="select_this form-control" cols="10" rows="5">
        &lt;input type="submit" class="btn btn-success btn-sm insert_<?php echo $rec_type;?>" value="Add <?php echo beautify_meta_name($rec_type); ?>"&gt;</textarea><?php } ?>
        </div>
<!-- Col 1 ends-->
...
$input_skeleton = '
<li class="item-content item-input {input_name}_container">
<div class="item-media {input_name}_media">
    <i class="f7-icons">chevron_right_square</i>
</div>
<div class="item-inner {input_name}_content">
  <div class="item-title item-label {input_name}_label">{input_name_human}</div>
  <div class="item-input-wrap">
    <input type="{input_type}" placeholder="{input_name_human}" class="{input_name}" name="{input_name}">    
  </div>
</div>
</li>';
$str_arr= ["{input_name}"=>$html_name,"{input_type}"=>$sub_type_html,"{input_name_human}"=>$human_readable];
foreach ($str_arr as $key => $value) {
    $input_skeleton = str_replace($key, $value, $input_skeleton);    
}
echo $input_skeleton; } ?>
<?php if($type == "h"){ 
    $input_skeleton = '
    <input type="hidden" class="{input_name} name="{input_name}">';
    $str_arr= ["{input_name}"=>$html_name,"{input_type}"=>$sub_type_html];
foreach ($str_arr as $key => $value) {
    $input_skeleton = str_replace($key, $value, $input_skeleton);    
}
    echo $input_skeleton;
...
<?php if($type == "t"){ 
    ...
    <div class="item-inner {input_name}_content">
      <div class="item-title item-label {input_name}_label">{input_name_human}</div>
      <div class="item-input-wrap">
      <textarea placeholder="{input_name_human}" class="{input_name}" name="{input_name}">&lt;/textarea> 
      </div>
    </div>
    </li>';
    $str_arr= ["{input_name}"=>$html_name,"{input_type}"=>$sub_type_html,"{input_name_human}"=>$human_readable];
...
<?php if($type == "s"){
   ...
   <div class="item-inner {input_name}_content">
     ...
     <div class="item-input-wrap input-dropdown-wrap">
                        <select id="{input_name}_select" name="{input_name}">
                          <option value="">Select</option>                          
                       ';
    ?>
     
            <?php foreach($get_options as $single_option){ 
                $human_readable_inside = ucwords(str_replace("_"," ",$single_option));
                if($single_option == "select"){
                    $single_option = "";
                }
                $input_skeleton .= '<option value="'.$single_option.'">'.$human_readable_inside.'</option>
                ';
                 }
                 $input_skeleton .= ' </select>
                 </div>
               </div>
             </li>';
             $str_arr= ["{input_name}"=>$html_name,"{input_type}"=>$sub_type_html,"{input_name_human}"=>$human_readable];
             ...
             echo $input_skeleton;
                    ...
                    <a class="button bg-color-app-primary button-fill margin insert_<?php echo $rec_type;?>">Submit</a> 
                </div>
                  </div>
              </div>
              &lt!-- Form Block Ends --&gt;
