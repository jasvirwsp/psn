                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title"><?php echo beautify_meta_name(title_case("template")); ?> <!-- Button trigger modal -->
<button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#add_template"><i class="fe-plus"></i> Add <?php echo beautify_meta_name(title_case("template"));?></button>
</h4>
                                </div>
                            </div>
                        </div>     
...
//Columns
$columns_to_list = array('template_mail_title');  // You can set it to "all", array("meta_name","another_meta_name"), blank
$columns_to_exclude = array("");
//Action Buttons
$edit_button_url = "edit_template.php?template_id=";
$view_button_url = "view_template.php?template_id=";
$delete_button_class = "delete_template";
//Records
$per_page = 20;
//Offset settings
...
$where_array = array("template_mail_title != ?"=>""); // You can change it as per required
...
<table class="table table-striped" id="list_table_template">
<?php
...
$dynamic_buttons = array(
  ["Design Template","email_builder/mail_builder.php?template_id=$record_id#no-sidebar","btn btn-sm w-100 mb-1 btn-warning","fe-activity","","_blank"]
);
//Dynamic Buttons Array
...
<!-- Col 1 -->
        ...
        <a href="javascript:void" class="btn btn-secondary btn-sm export_to_csv_template"><i class="fa fa-download" aria-hidden="true"></i> CSV</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm export_to_xls_template"><i class="fa fa-download" aria-hidden="true"></i> Excel</a>
         <a href="javascript:void" class="btn btn-secondary btn-sm print_table_template"><i class="fa fa-print" aria-hidden="true"></i> Print</a>
        </div>
<!-- Col 1 ends -->
</div>
<!-- Row Ends -->
...
<!-- Add template Start-->
<div class="modal fade" id="add_template" tabindex="-1" role="dialog" aria-labelledby="add_template_LongTitle" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Add <?php echo beautify_meta_name(title_case("template"));?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body">
      <!-- mail_title>i,mail_content>t --&gt<!-- Form Row Starts -->
        <div class="row">
        <form method="post" action="" id="form_template_rad5df56558448a3" class="w-100">
 <div class="form-group ic_template_mail_title col-md-5 d-inline-block">
         <label class="label_template_mail_title" for="template_mail_title">Mail Title</label>
        <input type="text" class="form-control template_mail_title" placeholder="Mail Title"  name="template_mail_title">
            </div>
    <div class="form-group ic_template_mail_content col-sm-12 d-none">
         <label class="label_template_mail_content" for="template_mail_content">Mail Content</label>
        <textarea class="form-control template_mail_content" placeholder="Mail Content" name="template_mail_content"></textarea>
        </div>
        
        <div class="form-group col-md-12">
        <input type="submit" class="btn btn-success btn-sm insert_template" value="Add Template" form_id="rad5df56558448a3"> 
        </div>
        </form>
        </div>
        <!-- Form Row Ends -->
...
<!-- Quick Edit Modal for template Start-->
<div class="modal fade" id="quick_edit_modal_template" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Update <?php echo beautify_meta_name(title_case("template")); ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">×</span>
        </button>
      </div>
      <div class="modal-body ic_age_modal">
      <form method="post" action="" id="form_update_template_quick_edit" class="w-100">
      <input type="hidden" name="record_id" value="" class="quick_edit_template_record_id">
      <div class="modal_input_loader_template">

      </div>
      <div class="form-group col-md-12">
       
        <input type="submit" class="btn btn-success btn-sm update_template" value="Update <?php echo beautify_meta_name(title_case("template")); ?>" form_id="quick_edit"> 
        </div>
        </form>
      </div>
      <div class="modal-footer">
...
<!-- Quick Edit Modal template Ends-->
        ...
        //Insert variables
        var insert_button_class_template = ".insert_template"; // .classname
                var form_id_template = "#form_template";  // #entry_form
                var insert_api_url_template = "api/template/insert_template.php"; // /api/insert.php
                var form_id_parameter_name = "form_id";
                
                //Insert Operation
                $(document).on("click",insert_button_class_template,function(e){                
                e.preventDefault();
                //Processing
                swal("Processing","Please Wait","warning");
                //Processing
                var form_to_process_template = form_id_template + "_" + $(this).attr(form_id_parameter_name);
        
                $.post(insert_api_url_template,$(form_to_process_template).serializeArray(),function(data){
              ...
              icon: "error"
                    ...
                    $(form_to_process_template + " input[type=text]").first().focus();
...
customized_content_template.innerHTML = "<a href='edit_template.php?template_id="+entry_id_template+"' class='btn btn-sm btn-warning mt-1'>Edit Template</a> <a href='#' onclick='location.reload()' class='btn btn-sm btn-warning mt-1'>Add New Template</a> <!--<a href='#' class='btn btn-sm btn-success mt-1'>Another Action</a> <a href='#' class='btn btn-sm btn-info mt-1'>Another Action</a> --><br><br><a href='#' onclick='var loc = location.href.replace(location.hash,\"\");window.location = loc;' class='btn btn-sm btn-danger'>Close & Refresh</a> <a href='#' onclick='swal.close()' class='btn btn-sm btn-danger'>X</a>";
swal({
    title: "Success",
...
var records_table_id = "#list_table_template";
$(".export_to_csv_template").click(function(){
$(records_table_id).tableToCSV();
});

$(".export_to_xls_template").click(function(){
...
$(".print_table_template").click(function(){
...
var delete_record_button_class_template = ".delete_template"; // .classname
var primary_record_key_attribute_name_template = "record_id"; //Name of attribute of particular record to edit/update/view/delete.
var delete_api_url_template = "api/template/delete_template.php"; // /api/delete.php

$(document).on("click",delete_record_button_class_template,function(e){
e.preventDefault();
...
.then((willDelete) => {
if (willDelete) {
  var record_id_template = $(this).attr(primary_record_key_attribute_name_template);
    
  $.post(delete_api_url_template,{"record_id":record_id_template});
...
.then((willDelete) => {
  ...
  var api_url = "api/template/delete_template.php"; //Replace with correct API url
...
//Quick Edit Code for template starts
$(".quick_edit_template").click(function(){
          var quick_edit_template_record_type = 'template';
          var quick_edit_template_record_id = $(this).attr("record_id");
          $(".quick_edit_template_record_id").val(quick_edit_template_record_id);
          var quick_edit_template_input_name = $(this).attr("quick_edit_input_name");
          $("#quick_edit_modal_template").modal("show");
          $(".modal_input_loader_template").load("edit_"+quick_edit_template_record_type+".php?"+quick_edit_template_record_type+"_id="+quick_edit_template_record_id+" .ic_"+quick_edit_template_input_name);
        });
        

//Update Variables for template
var qe_update_button_class_template= ".update_template"; // .classname
var qe_form_id_template = "#form_update_template";
var qe_update_api_url_template = "api/template/update_template.php"; // /api/update.php
 var qe_form_id_parameter_name_template = "form_id";
    //Update Operation
    	   $(document).on("click",qe_update_button_class_template,function(e){
e.preventDefault();
    		  var qe_form_to_process_template = qe_form_id_template + "_" + $(this).attr(qe_form_id_parameter_name_template);

    	 $.post(qe_update_api_url_template,$(qe_form_to_process_template).serializeArray(),function(data){
        var qe_response_json_template = data;

if(qe_response_json_template["status"] == "failure"){
  var qe_get_errors_template = qe_response_json_template["errors"];
 
    var errors_returned_template = "";
  $(qe_get_errors_template).each(function(index,value){
            errors_returned_template = errors_returned_template + " => " + value;
    });
    ...
    swal("Error",errors_returned_template, {
      icon: "error"
        ...
        //Reset Form
    		//$(qe_form_to_process_template)[0].reset();

        

    		 // Other functions to execute on success
    		   //Customized buttons
var qe_customized_content_template = document.createElement("div");
qe_customized_content_template.innerHTML = "<a href='javascript:void(0)' onClick='location.reload()' class='btn btn-sm btn-warning mt-1'>Refresh Changes</a> <br><br><a href='javascript:void(0)' onclick='swal.close()' class='btn btn-sm btn-danger'>Close Box</a>";
swal({
    title: "Updated",
    text: "Choose next action",
  content: qe_customized_content_template,
  buttons: false,
  icon : 'success',
});
//Customized buttons
...
//Quick Edit Code for template ends 
        </script>
